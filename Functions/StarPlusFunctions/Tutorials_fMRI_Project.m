...
% 
 ...
 [result,predictedLabels,trace] = summarizePredictions(predictions,classifier,'accuracy',labels); % 'averageRank','error','accuracy'
...
% draw a picture of activation for the z=4 slice of the brain for trial 4, at time 8
   plotSnapshot(info,data,meta,4,4,8,0,0);  % , 0,0) the colors wil depend on data 

% do the same as above, but for all slices of the brain
   plotSnapshot3D(info,data,meta,2,1,0,0);
...
% plot the time course for just trials 3 and 4 of voxel <36,62,8>
   [i,d,m]=transformIDM_selectTrials(info,data,meta,[3,4,20,1]);
